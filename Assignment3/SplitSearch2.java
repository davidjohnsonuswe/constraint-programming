...
public class SplitSearch2  {
	...
	IntVar selectVariable(IntVar[] v) {
	    if (v.length != 0) {
			IntDomain actualDomain = v[v.length-1].dom();

		if (actualDomain.getSize() == 1){
		...
		// v[v.length-1] is not fixed. Choices can still be made
			searchVariables = v;
		}
		return v[v.length-1];
	...
	int selectValue(IntVar v) {
		IntDomain actualDomain = v.domain;
		int c = (actualDomain.min() + actualDomain.max())%2==0 ? (actualDomain.min() + actualDomain.max())/2 : (actualDomain.min() + actualDomain.max()+1)/2;
		return c;
	}
